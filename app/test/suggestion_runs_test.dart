...
void main() {
  ...
  setUp(() async {
    ...
    obs = await (db.select(db.observations)
  ...
  final liveOak = IdCandidate(
      name: 'Quercus fusiformis', commonName: 'Plateau live oak',
      score: 0.72, source: 'plantnet');
  final cedar = IdCandidate(
      name: 'Juniperus ashei', commonName: 'Ashe juniper',
      score: 0.18, source: 'plantnet');

  test('two runs stack with distinct run ids; accept flags only its run',
      () async {
    final svc = IdentificationService(db);
    final run1 =
        await svc.recordSuggestions([liveOak, cedar], obs);
    ...
    await svc.accept(
        ...
        propertyId: propId,
        runId: run2);
    final accepted = await (db.select(db.identificationSuggestions)
          ..where((s) => s.accepted.equals(1)))
        .get();
    ...
    final after = await (db.select(db.observations)
  ...
  test('accept without a run id keeps the old by-name behaviour', () async {
    ...
    await svc.accept(
        candidate: liveOak, observationId: obs.id, propertyId: propId);
    final accepted = await (db.select(db.identificationSuggestions)
          ..where((s) => s.accepted.equals(1)))
        .get();
    expect(accepted, hasLength(1));
