...
void main() {
  ...
  setUp(() async {
    ...
    property = await (db.select(
      db.properties,
    )..where((x) => x.id.equals(propId))).getSingle();
    ...
    await db
        ...
        .insert(
          ZonesCompanion.insert(
            ...
            geojson: '{"type":"Polygon","coordinates":[[[-98.2,31.0],[-98.15,31.0],[-98.15,31.05],[-98.2,31.05],[-98.2,31.0]]]}',
            createdBy: 'austin',
            createdAt: now,
            updatedAt: now,
          ),
  ...
  test('produces the full export layout with correct contents', () async {
    ...
    expect(
      File(p.join(dir.path, 'database.sqlite')).lengthSync(),
      greaterThan(0),
    );

    // CSV: header + 3 live rows, no tombstoned row; quoting survives.
    final obsCsv = File(p.join(dir.path, 'data', 'observations.csv'))
        .readAsLinesSync();
    ...
    final obsGeo = jsonDecode(
      File(p.join(dir.path, 'geo', 'observations.geojson')).readAsStringSync(),
