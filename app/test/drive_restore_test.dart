...
class _FakeDrive {
  ...
  http.Client client() => MockClient((request) async {
        ...
        if (path.startsWith('/upload/drive/v3/files')) {
          ...
          final id =
              request.method == 'PATCH' ? path.split('/').last : 'id${_next++}';
...
void main() {
  ...
  Future<List<Uint8List>> seedData() async {
    ...
    for (var i = 0; i < 3; i++) {
      final bytes =
          Uint8List.fromList(List.generate(40000 + i, (j) => (i + j) % 251));
      photos.add(bytes);
  ...
  test('plain: backup to Drive, stage a restore back out of it', () async {
    ...
    final staged =
        sqlite.sqlite3.open('${docsDir.path}/restore_staged/db.sqlite');
    try {
    ...
    for (final original in photos) {
      expect(restored.any((r) => r.length == original.length), isTrue,
          reason: 'a restored blob matches each original size');
    }
  });

  test('encrypted: restores with the passphrase, refuses the wrong one',
      () async {
    await seedData();
    final keyring = await BackupKeyring.create('correct horse',
        memoryKiB: 256, iterations: 1);
    final target = DriveTarget(accessToken: 't', client: drive.client());
    final engine = BackupEngine(db, target, keyring.cipher,
