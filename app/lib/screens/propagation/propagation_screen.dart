...
class PropagationScreen extends StatelessWidget {
  ...
  Future<void> _newBatch(BuildContext context) async {
        ...
        child: ListView(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: const [
                DropdownMenuItem(
                    value: 'water_rooting', child: Text('Water rooting')),
                DropdownMenuItem(
                    value: 'perlite_coir', child: Text('Perlite/coir')),
                DropdownMenuItem(
                    value: 'direct_stick', child: Text('Direct stick')),
                DropdownMenuItem(
                    value: 'flood_tray', child: Text('Flood tray')),
                DropdownMenuItem(
                    value: 'cold_moist_strat',
                    child: Text('Cold moist stratification')),
                DropdownMenuItem(
                    value: 'warm_strat', child: Text('Warm stratification')),
                DropdownMenuItem(
                    value: 'scarification', child: Text('Scarification')),
                DropdownMenuItem(
                    value: 'direct_sow', child: Text('Direct sow')),
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Mother plant (optional, e.g. Riverbank willow #3)',
                helperText:
    ...
    if (sourceLabel.isNotEmpty) {
      ...
      await db.into(db.sourcePlants).insert(SourcePlantsCompanion.insert(
            id: sourcePlantId,
      ...
      await db
          ...
          .insert(CollectionEventsCompanion.insert(
    ...
    await db
        ...
        .insert(PropagationBatchesCompanion.insert(
          id: newId(),
          propertyId: property.id,
          collectionEventId: Value(collectionEventId),
          taxonId: Value(taxon?.id),
          batchCode: Value(codeController.text.trim().isEmpty
              ? null
              : codeController.text.trim()),
          startedOn: today,
          method: Value(method),
          container: Value(containerController.text.trim().isEmpty
              ? null
...
class _BatchTile extends StatelessWidget {
  ...
  Future<String> _species() async {
    ...
    final t = await (db.select(db.taxa)
          ..where((x) => x.id.equals(batch.taxonId!)))
        .getSingleOrNull();
    ...
    return FutureBuilder<String>(
      ...
      builder: (context, snapshot) {
        ...
        return ListTile(
          ...
          subtitle: Text('${batch.status ?? 'active'} · '
