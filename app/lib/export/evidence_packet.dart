  ...
  Future<Uint8List> build(Property property, Program program) async {
    ...
    final activities = practices.isEmpty
        ...
        : await (db.select(db.practiceActivities)
              ..where((x) => x.practiceId.isIn([for (final p in practices) p.id]))
              ..where((x) => x.deletedAt.isNull())
    ...
    if (from != null) obsQ = obsQ..where((o) => o.observedAt.isBiggerOrEqualValue(from));
    if (to != null) {
      obsQ = obsQ..where((o) => o.observedAt.isSmallerOrEqualValue('${to}T23:59:59Z'));
    }
    ...
    if (observations.isNotEmpty) {
      final links = await (db.select(db.mediaLinks)
            ...
            ..where((l) =>
                l.entityId.isIn([for (final o in observations) o.id]))
    ...
    pw.Widget label(String t) => pw.Text(t.toUpperCase(),
        style: pw.TextStyle(
            fontSize: 7, letterSpacing: 1.4, color: _soft));
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        // ---- Header ----
        pw.Text('EVIDENCE PACKET',
            style: pw.TextStyle(
                fontSize: 9, letterSpacing: 3, color: _soft)),
        ...
        pw.Row(children: [
          pw.Expanded(
              child: pw.Column(
                  ...
                  children: [
                label('Property'),
                pw.Text(property.name, style: const pw.TextStyle(fontSize: 11)),
              ])),
          pw.Expanded(
              child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
            ...
            pw.Expanded(
                child: pw.Column(
                  ...
                  pw.Text(
                      [
                        program.contactName!,
                        if (program.contactEmail != null) program.contactEmail!,
                      ].join('\n'),
                      style: const pw.TextStyle(fontSize: 11)),
                ])),
        ]),
        ...
        if (practices.isEmpty)
          pw.Text('No practices recorded against this program.',
        ...
        if (practices.isNotEmpty)
          pw.TableHelper.fromTextArray(
            ...
            headerDecoration:
                const pw.BoxDecoration(color: PdfColor.fromInt(0xFFEDE7D8)),
            cellPadding:
                const pw.EdgeInsets.symmetric(horizontal: 5, vertical: 4),
            headers: ['Code', 'Practice', 'Zone', 'Planned', 'Done', 'Status', 'Due / done'],
            data: [
            ...
            children: [
              for (final (date, f) in photoFiles)
                pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                      pw.Image(pw.MemoryImage(f.readAsBytesSync()),
                          width: 156, height: 117, fit: pw.BoxFit.cover),
                      pw.SizedBox(height: 2),
                      pw.Text(date,
                          style: pw.TextStyle(fontSize: 7, color: _soft)),
                    ]),
